...
import numpy as np
from vectorlite import VectorLite, Document, VectorLiteRetriever
...
#     results = vector_db_retriever._get_relevant_documents(query_text, k=25)
...
def ranker(query_text):
    ...
    for (idx,doc) in results:
        opportunity_id = Opportunity.query.get(doc.metadata['id'])
        top_opportunities.append((opportunity_id, doc.metadata))
